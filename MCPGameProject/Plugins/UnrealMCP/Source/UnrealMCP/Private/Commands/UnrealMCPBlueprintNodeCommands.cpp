...
TSharedPtr<FJsonObject> FUnrealMCPBlueprintNodeCommands::HandleCommand(const FString& CommandType, const TSharedPtr<FJsonObject>& Params)
{
    ...
    else if (CommandType == TEXT("add_blueprint_custom_event"))
    {
        return HandleAddBlueprintCustomEvent(Params);
    }
    else if (CommandType == TEXT("remove_blueprint_node"))
    {
        return HandleRemoveBlueprintNode(Params);
    }
    ...
    else if (CommandType == TEXT("add_blueprint_set_variable_on_object"))
    {
        return HandleAddBlueprintSetVariableOnObject(Params);
    }
...
TSharedPtr<FJsonObject> FUnrealMCPBlueprintNodeCommands::HandleConnectBlueprintNodes(const TSharedPtr<FJsonObject>& Params)
{
    ...
    // Get the event graph
    UEdGraph* EventGraph = ResolveGraph(Blueprint, Params);
    if (!EventGraph)
    {
...
TSharedPtr<FJsonObject> FUnrealMCPBlueprintNodeCommands::HandleAddBlueprintVariableGetNode(const TSharedPtr<FJsonObject>& Params)
{
    ...
    UK2Node_VariableGet* VarNode = FUnrealMCPCommonUtils::CreateVariableGetNode(EventGraph, Blueprint, VariableName, Position);
    if (!VarNode)
    {
    ...
    TSharedPtr<FJsonObject> ResultObj = MakeShared<FJsonObject>();
    ResultObj->SetStringField(TEXT("node_id"), VarNode->NodeGuid.ToString());
    ResultObj->SetStringField(TEXT("variable_name"), VariableName);
    return ResultObj;
}
...
TSharedPtr<FJsonObject> FUnrealMCPBlueprintNodeCommands::HandleAddBlueprintCustomEvent(const TSharedPtr<FJsonObject>& Params)
{
    FString BlueprintName;
    if (!Params->TryGetStringField(TEXT("blueprint_name"), BlueprintName))
    {
...
TSharedPtr<FJsonObject> FUnrealMCPBlueprintNodeCommands::HandleRemoveBlueprintNode(const TSharedPtr<FJsonObject>& Params)
{
    FString BlueprintName;
    if (!Params->TryGetStringField(TEXT("blueprint_name"), BlueprintName))
    {
...
UEdGraph* FUnrealMCPBlueprintNodeCommands::ResolveGraph(UBlueprint* Blueprint, const TSharedPtr<FJsonObject>& Params)
{
    FString GraphName;
...
TSharedPtr<FJsonObject> FUnrealMCPBlueprintNodeCommands::HandleOverrideBlueprintFunction(const TSharedPtr<FJsonObject>& Params)
{
    FString BlueprintName;
    if (!Params->TryGetStringField(TEXT("blueprint_name"), BlueprintName))
    {
    ...
    if (ExistingGraph)
    {
        // Already exists, return info about it
        TSharedPtr<FJsonObject> ResultObj = MakeShared<FJsonObject>();
        ResultObj->SetStringField(TEXT("graph_name"), ExistingGraph->GetName());
        ResultObj->SetBoolField(TEXT("already_existed"), true);

        // Find entry and return nodes
    ...
    {
        Function = SearchClass->FindFunctionByName(FName(*FunctionName), EIncludeSuperFlag::ExcludeSuper);
        SearchClass = SearchClass->GetSuperClass();
    }

    if (!Function)
    {
        return FUnrealMCPCommonUtils::CreateErrorResponse(FString::Printf(TEXT("Function not found in parent hierarchy: %s"), *FunctionName));
    }

    // Create the override graph using the engine utility
    ...
    FBlueprintEditorUtils::AddFunctionGraph(Blueprint, NewGraph, false, Function->GetOwnerClass());
    FBlueprintEditorUtils::MarkBlueprintAsModified(Blueprint);

    // Return info about the created graph
    TSharedPtr<FJsonObject> ResultObj = MakeShared<FJsonObject>();
    ResultObj->SetStringField(TEXT("graph_name"), NewGraph->GetName());
    ResultObj->SetBoolField(TEXT("already_existed"), false);
...
TSharedPtr<FJsonObject> FUnrealMCPBlueprintNodeCommands::HandleAddBlueprintSetVariableOnObject(const TSharedPtr<FJsonObject>& Params)
{
    FString BlueprintName;
    if (!Params->TryGetStringField(TEXT("blueprint_name"), BlueprintName))
    {
    ...
    UClass* TargetClass = nullptr;
    TArray<FString> NamesToTry;
    NamesToTry.Add(TargetClassName);
    if (!TargetClassName.StartsWith(TEXT("U")) && !TargetClassName.StartsWith(TEXT("A")))
    {
        NamesToTry.Add(TEXT("U") + TargetClassName);
        NamesToTry.Add(TEXT("A") + TargetClassName);
    }
    for (const FString& Name : NamesToTry)
    {
        TargetClass = FindObject<UClass>(ANY_PACKAGE, *Name);
        if (TargetClass) break;
    }
    if (!TargetClass)
    {
        UBlueprint* TargetBP = FUnrealMCPCommonUtils::FindBlueprint(TargetClassName);
    ...
    if (!Property)
    {
        return FUnrealMCPCommonUtils::CreateErrorResponse(FString::Printf(TEXT("Property '%s' not found on class '%s'"), *VariableName, *TargetClassName));
    }

    FVector2D Position = FUnrealMCPCommonUtils::GetVector2DFromJson(Params, TEXT("node_position"));

    UK2Node_VariableSet* SetNode = NewObject<UK2Node_VariableSet>(Graph);
    SetNode->VariableReference.SetExternalMember(FName(*VariableName), TargetClass);
    SetNode->NodePosX = Position.X;
    SetNode->NodePosY = Position.Y;
    Graph->AddNode(SetNode, true);
